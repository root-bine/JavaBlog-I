...
`UnicodeBlock`是**`一系列字符子集`**，用于表示**`Unicode字符的块范围`**。
...
| `>>`（右移）        |          左操作数<u>按位右移</u>右操作数指定的位数           |              60>>3=7（0111）              |
| `<<`（左移）        |          左操作数<u>按位左移</u>右操作数指定的位数           |          60<<2=240（1111 0000）           |
| `>>>`（无符号右移） | 左操作数的值按右操作数指定的位数右移，移动得到的空位以零填充 |          60>>>2=15（0000 1111）           |
